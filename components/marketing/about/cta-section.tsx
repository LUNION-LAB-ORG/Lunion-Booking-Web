...
import { IconChartBar, IconChevronRight, IconFile, IconGlobe, IconHeartHandshake, IconRss, IconShield } from '@tabler/icons-react';
...
const tiles = [
    {
        icon: <IconHeartHandshake className="size-full" />,
        bg: (
    ...
    {
        icon: <IconGlobe className="size-full" />,
        bg: (
    ...
    {
        icon: <IconFile className="size-full" />,
        bg: (
    ...
    {
        icon: <IconShield className="size-full" />,
        bg: (
    ...
    {
        icon: <IconRss className="size-full" />,
        bg: (
            <div className="pointer-events-none absolute left-1/2 top-1/2 h-1/2 w-1/2 -translate-x-1/2 -translate-y-1/2 overflow-visible rounded-full bg-gradient-to-r from-orange-600 via-rose-600 to-violet-600 opacity-70 blur-[20px] filter" />
        ),
    },
    {
        icon: <IconChartBar className="size-full" />,
        bg: (
                    ...
                    <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
                        ...
                        <div className="absolute z-10">
                            <div className="mx-auto size-24 rounded-[2rem] border bg-white/10 p-3 shadow-2xl backdrop-blur-md dark:bg-black/10 lg:size-32">
                                <IconHeartHandshake className="mx-auto size-16 text-black dark:text-white lg:size-24" />
                            </div>
                            <div className="z-10 mt-4 flex flex-col items-center text-center text-primary">
                                <h1 className="text-3xl font-bold lg:text-4xl">Prêt à booster votre activité immobilière ?</h1>
                                <p className="mt-2">Commencez votre essai gratuit de notre outil de travail de 30 jours. Aucune carte de crédit requise.</p>
                                <Button
                                    as={Link}
                                    className="group mt-4 rounded-[2rem] px-6"
                                    color="primary"
                                    endContent={<IconChevronRight className="ml-1 size-4 transition-all duration-300 ease-out group-hover:translate-x-1" />}
                                    href="/"
                                    variant="bordered"
                                >
